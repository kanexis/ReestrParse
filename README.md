...
> **v0.9.2:** runtime Spread-reader больше не сдвигает значения при пустых параметрах: пустая formula/value-cell остаётся пустой, а не подменяется текстом справа. Явный номер формы сильнее эвристики по пересекающимся кодам; поля дополнительно привязаны к смысловым label. DevExpress pager делает до 3 click-попыток и при зависшем callback один раз восстанавливает отфильтрованный каталог.
>
> **v0.9.1:** исправлена гонка первой страницы после `НАЙТИ`, worker-ы сами подтверждают фильтры перед поиском строки, а новая Canvas/Spread форма 1 читается через runtime JS API. Поддерживаются 4.1.1, 1 и 1.0.1. До 10 worker-ов (по умолчанию 6).
...
## Retry открытия карточки организации

Если строка в DevExpress-каталоге найдена и нажата, но браузер не переходит в карточку, Worker не завершает организацию сразу. Он делает до 5 полных попыток: возвращается на catalog URL, повторно применяет фильтры, ждёт стабильный grid, переходит на нужную страницу, заново находит строку и повторяет клик.

Телеметрия: `CATALOG_ORG_OPEN_ATTEMPT_FAILED`, `CATALOG_ORG_OPEN_RETRY`, `CATALOG_ORG_OPENED`.
...
8. На странице организации собирает все опубликованные `TemplatePrinter.aspx` URL и приоритизирует **4.1.1**, затем современную **форму 1**, затем **1.0.1** и остальные опубликованные workbook-кандидаты.
9. Не кликает модальное окно/iframe: прямой URL извлекается из `openTemplateDialog(...)`.
10. TemplatePrinter разбирается двумя способами: старый HTML-table и новый GrapeCity/Wijmo Spread/Canvas через live JS API. HTML workbook разбирается по нескольким листам: **4.1.1** даёт контакты, **1.0.1** — систему, вид деятельности, регион/район/муниципальное образование и дату обновления.
...
### Современная форма 1 — основной источник контактов, если 4.1.1 отсутствует

Для теплоснабжения по действующей форме используются коды: `1` — наименование; `6.1–6.3` — ФИО руководителя; `7` — почтовый адрес; `8` — адрес местонахождения; `9` — телефоны; `10` — сайт; `11` — email. На новом TemplatePrinter значения рисуются на Canvas, поэтому читаются из runtime Spread-модели, а не из `PageSource`.
