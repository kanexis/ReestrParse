...
## Классический интерфейс + отчёты (v0.9.8)

В v0.9.8 возвращён внешний вид v0.9.6: тёмная боковая панель с pipeline, единая панель параметров, большая таблица и нижняя строка прогресса. Возможности Excel и пакетной обработки из v0.9.7 сохранены и встроены в этот интерфейс без отдельного редизайна.

Главная таблица показывает только итоговые поля реестра: **Наименование**, **ИНН**, **Email** и флажок **В отчёт**. Техническая диагностика не удаляется — она доступна в раскрытии выбранной строки и в мониторинге.

## Итоговый Excel-реестр (v0.9.7)

После сбора карточек ReestrParse может сформировать XLSX напрямую из приложения. В публичный реестр выводятся только три поля: **Наименование**, **ИНН**, **Email**. Email выбирается по приоритету: руководитель → организация → ответственное лицо.

Доступны два режима:

1. **Отдельный файл для региона** — `Реестр_<регион>_Теплоснабжение_<дата>.xlsx`.
2. **Один общий Excel** — `Реестр_Теплоснабжение_<дата>.xlsx`, регионы сохраняются на отдельных листах.

Папка выбирается в UI. Можно исключать строки без email, ошибочные организации или вручную снять флажок **«В отчёт»** у конкретной организации.

### Все регионы

Пакетный режим проходит регионы строго **последовательно**: каталог региона → карточки/forms с выбранным количеством workers → запись результата → следующий регион. Доступны безопасная пауза/продолжение, пропуск текущего региона и продолжение после ошибки. Пауза применяется на ближайшей безопасной точке между тяжёлыми Selenium-этапами; уже выполняющийся callback/карточка не замораживается посередине.

### Мониторинг

Окно мониторинга по умолчанию показывает только ключевые события, warnings/errors, retry/backoff, пакетные события и сохранение Excel. Переключатель **«Подробный лог»** показывает полный telemetry поток; CSV-экспорт также сохраняет полный журнал.



> **v0.9.6:** основной catalog crawler снова ходит строго последовательно через штатную DevExpress кнопку `Следующая` (`PBN`). Сложные `visible pager hops` оставлены только details-workers для далёких `SourcePage`, поэтому worker-fix больше не вмешивается в стабильный обход `1 -> 2 -> 3 -> ...`.

> **v0.9.5:** DevExpress pager теперь ходит через ближайшие реально видимые numeric-link. Для типичного перехода `1 → 8`, когда `8` ещё скрыта многоточием, Worker сначала нажимает ближайшую видимую `7`, перечитывает pager после callback и затем нажимает появившуюся `8`.

> **v0.9.4:** details-workers используют DevExpress client API для дальних переходов страниц. При сбое `GotoPage` выполняется последовательный fallback, а серия одинаковых pager failures включает общий короткий backoff, чтобы не создавать request storm.
...
- [`docs/reporting.md`](docs/reporting.md) — XLSX-отчёты, фильтрация строк и последовательный режим всех регионов.
...
- выбор числа parallel workers `1..10` (по умолчанию 6);
- фоновые Chrome для details phase;
- live-progress;
- поиск по организации, ИНН, приоритетному email, источнику email и статусу;
- компактная итоговая таблица `Наименование / ИНН / Email` + флажок `В отчёт`;
- диагностические сведения выбранной организации через `RowDetails` без раздувания основной таблицы;
- два XLSX-режима: отдельный файл региона или один workbook с листами регионов;
- последовательная обработка всех регионов с паузой, пропуском текущего региона и отменой;
...
Контактные данные обрабатываются параллельно. По умолчанию UI использует **6 workers**; доступно `1..10`. Для длительных пакетных прогонов разумно начинать с `4–6` и повышать только если ЕИАС стабильно обслуживает callback-нагрузку. Общий backoff защищает от retry-storm при серии однотипных сбоев DevExpress.
